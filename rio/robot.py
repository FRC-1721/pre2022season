...
class UnnamedToaster(wpilib.TimedRobot):
    ...
    def teleopPeriodic(self):
        ...
        if self.joy.getRawAxis(1) < 0:
            joyVectorAngle = vectorMath(
                -self.joy.getRawAxis(1), self.joy.getRawAxis(0), 0
            )
        elif self.joy.getRawAxis(1) > 0:
            joyVectorAngle = vectorMath(
                -self.joy.getRawAxis(1), self.joy.getRawAxis(0), 1
            )
        else:
        ...
        self.joyPercent = (
            self.joyVectorAngle / 360
        )  # Converts degrees to a percentage of turning, with 0 being up, 25 is right, 50 is down, 75 is left, etc.
        self.currentPosition = (
            self.spinEncoder.getPosition() % 11
        ) / 11  # Get the current position of the wheel as a percent, witht the same methodology as above
        self.movePercent = (
            self.joyPercent - self.currentPosition
        )  # Difference between the current position and the joysticks position
        self.neoTurns = (
            self.movePercent * self.neoTurnRatio
        )  # how many times the neo has to turn to reach the joystick position
